...
compare_dwa_mau <- function(df,item_name,path_output,write_res=NULL,n_table=5){
  ...
  df <- hndl_UC(df,8:10)
  stop <- Sys.time()
  cat("Time needed for step handle UC",sep="\n")
  print(stop-start)
  # check input data
  ...
  stop <- Sys.time()
  cat("Time needed for step MergeMulti",sep="\n")
  print(stop-start)
  ...
  if(write_res == T){
    # write df_compare
    write.xlsx(phn_lex$df_compare,file.path(path_output,paste0(item_name,"_compare.xlsx")))
  }
  # return
...
compare_PhonByLexCl <- function(df,n_freq){
  ...
  for(i in 1:n_freq){
    ...
    # print(paste0(df$dwa_phontype[vec_row[v]], "_" ,df$mau_phontype[vec_row[v]]))
      ...
      df$phn_class_abs[vec_row[v]]  <- df$phontype_cleanLex[vec_row[v]]
      df$reliability[vec_row[v]] <-max(mat)  
    ...
    n_entries_ry<-length(vec_row)
    reliability <-round(sum(df$reliability[vec_row])/length(vec_row),digits = 4)
    # save results
    res <-cbind(lex_name,n_entries,similarity,n_entries_ry,reliability)
    } # end if lexclass==0
    # else set reliablity to NA
     if(length(vec_row)==0){
       lex_name <- names(tbl[i])
       n_entries <- length(which(df$dwa_lextype==names(tbl[i]) & df$lextype_compare_abs==1))
       similarity <- round(sum(df$phontype_compare_abs[which(df$dwa_lextype==names(tbl[i]) & df$lextype_compare_abs==1)])/
